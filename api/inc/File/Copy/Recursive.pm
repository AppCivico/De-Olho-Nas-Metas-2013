...
BEGIN {
    ...
    $INC{'warnings.pm'} = "fake warnings entry for < 5.6 perl ($])"
      if $] < 5.006;
}
use warnings;
...
use File::Copy;
use File::Spec
  ; #not really needed because File::Copy already gets it, but for good measure :)
...
@ISA = qw(Exporter);
@EXPORT_OK =
  qw(fcopy rcopy dircopy fmove rmove dirmove pathmk pathrm pathempty pathrmdir);
$VERSION = '0.38';
...
my $samecheck = sub {
    return 1
      if $^O eq 'MSWin32';    # need better way to check for this on winders...
    ...
    if ( -d $_[0] && !$CopyLoop ) {
        ...
        while (@pth) {
            ...
            last
              if
              !$cur;  # probably not necessary, but nice to have just in case :)
...
my $move = sub {
    ...
    if (@x) {
        ...
        if ($RemvBase) {
            ...
            pathrm( File::Spec->catpath( $volm, $path, '' ),
                $ForcePth, $NoFtlPth )
              or return;
        }
    }
...
sub fcopy {
    ...
    if ( -l $_[0] && $CopyLink ) {
        ...
        my $mode_trg =
          -d $_[1]
          ? File::Spec->catfile( $_[1], $base_file[$#base_file] )
          : $_[1];
...
sub pathmk {
    ...
    for ( $zer .. $#parts ) {
        ...
        $pth = File::Spec->catdir( $pth, $parts[ $_ + 1 ] )
          unless $_ == $#parts;
    }
    1;
}
