package WebSMM::Controller::Admin::Category;
use Moose;
...
sub base : Chained('/admin/base') : PathPart('category') : CaptureArgs(0) {
}
...
sub index : Chained('base') : PathPart('') : Args(0) {
    ...
    $c->stash->{count_partial} 	= scalar keys $c->stash->{categories};
	$c->stash->{total}   		= $c->stash->{count};
	$c->stash->{results} 		= $c->stash->{categories};
