...
sub download : Chained('base') : PathPart('subprefecture') : CaptureArgs(0) {
    my ( $self, $c ) = @_;

    @{ $c->stash->{header} } = (
        "nome", "sigla",       "latitude", "longitude",
        "site", "subprefeito", "email",    "telefone",
        "endereço",
    );
}
...
sub upload : Chained('base') : PathPart('upload_subprefecture') : Args(0) {
    my ( $self, $c ) = @_;
    my $api = $c->model('API');
