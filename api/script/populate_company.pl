...
for my $b (@budgets){
	my $comp = $schema->resultset('Company')->search({ name => $b->business_name })->next;

	my $bud = $schema->resultset('Company')->create({ name => $b->business_name, name_url => $b->business_name_url, goal_id => $b->goal_number}) 	unless $comp;
